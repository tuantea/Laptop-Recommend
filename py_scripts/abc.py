...
gaming_laptops_df = pd.read_csv("../data/abc.csv", sep=",")
...
def clean_memory():
    ...
    for i in range(len(gaming_laptops_df['memory'])):
        ...
        if gaming_laptops_df.loc[i,"memory"] == -1:
            ...
            gaming_laptops_df.loc[i, 'memory'] = int(gaming_laptops_df.loc[i, 'memory'].split('G')[0])
...
def clean_weight():
    # retrieve GB ints from raw data (truy xuất GB ints từ dữ liệu thô)
    for i in range(len(gaming_laptops_df['weight'])):
        #if type(gaming_laptops_df.loc[i, 'memory']) == int or type(gaming_laptops_df.loc[i, 'memory']) == 'numpy.64int':
            #continue
        if gaming_laptops_df.loc[i,"weight"] == -1:
            gaming_laptops_df.loc[i,"weight"] = 1
        else:
            gaming_laptops_df.loc[i, 'weight'] = float(gaming_laptops_df.loc[i, 'weight'].split('k')[0])
## MAIN
# clean_prices()
# clean_ratings()
# clean_cpu_speed()
...
gaming_laptops_df.to_csv("laptop_cleaned.csv")